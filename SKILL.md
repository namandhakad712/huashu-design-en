...
description: Huashu-Design — HTML-based hi-fi prototypes, interactive demos, slide decks, motion design, design variation exploration + design direction advisor + expert critique + voiceover narration. HTML is the tool, not the medium. Embodies different experts (UX designer/animator/slide designer/prototypist) based on task to avoid web design tropes. Triggers: prototype, design demo, interactive prototype, HTML demo, animation demo, design variations, hi-fi design, UI mockup, prototype, design exploration, make an HTML page, make a visualization, app prototype, iOS prototype, mobile app mockup, export MP4, export GIF, 60fps video, design style, design direction, design philosophy, color scheme, visual style, recommend style, pick a style, make it look good, critique, review this design, voiceover, narration, animation with voiceover, explainer video, concept video, long video, TTS animation, 5-minute explainer. **Core capabilities**: Junior Designer workflow (show assumptions + reasoning + placeholders first, then iterate), Anti-AI slop checklist, React+Babel best practices, Tweaks variation switching, Speaker Notes, Starter Components (slide shell/variation canvas/animation engine/device frame/NarrationStage), App prototype specific rules (default to real images from Wikimedia/Met/Unsplash, each iPhone wraps AppPhone state manager for interactivity, run Playwright click test before delivery), Playwright verification, HTML animation → MP4/GIF export (25fps base + 60fps interpolation + palette-optimized GIF + 6 scene-specific BGM tracks + auto fade), **voiceover-driven animation pipeline** (Doubao TTS generates voiceover + real-duration timeline.json + NarrationStage drives visuals + ducking-mixed final MP4; iron rule: entire piece is one continuous motion narrative, no PowerPoint-style cuts). **Fallback when requirements are vague**: Design Direction Advisor mode — recommend 3 differentiated directions from 5 schools × 20 design philosophies (Pentagram information architecture/Field.io kinetic poetry/Kenya Hara Eastern minimalism/Sagmeister experimental, etc.), display 24 pre-made showcases (8 scenes × 3 styles), generate 3 visual demos in parallel for user selection. **Optional post-delivery**: Expert 5-dimension critique (philosophy consistency/visual hierarchy/detail execution/functionality/innovation each scored 10/10 + fix list).
---
...
9.5. **(When narration needed) Voiceover-driven animation · L2 long concept video**: When user wants "explain a concept in 5-20 minutes", "tutorial with voiceover", "long-form科普 video" — **don't make animation first then add voiceover**, that causes mismatch between visual rhythm and narration. Use `references/voiceover-pipeline.md` instead:
   - **Write script** (markdown, `## scene-id` sections, `[[cue:xx]]` markers for key moments) — script is the source code, rhythm depends on it
   - **Run narrate-pipeline.mjs** (Doubao TTS · configure voice in `.env`) → output voiceover.mp3 + timeline.json (cue times are real-measured, not estimated by character count)
   - **🛑 Answer 3 iron rules before animating**: (1) What is the hero element? (2) How does it morph across scenes? (3) Does every frame have motion? If can't answer, don't write code
   - **Write animation HTML**: Use `assets/narration_stage.jsx` (NarrationStage + Scene + Cue + useNarration + useSceneFade + **Subtitles**) → hero goes directly as child of `<NarrationStage>`, not inside `<Scene>`; `<Subtitles />` included by default (Bilibili style · dark ink + white glow, auto-splits to ≤12 Chinese chars per line, never crosses sentence boundaries)
   - **Render final MP4**: `bash scripts/render-narration.sh demo.html --timeline=_narration/timeline.json [--bgm-mood=educational]` → auto records silent MP4 + mixes voiceover + optional BGM
   - **Failure mode #1 (must avoid)**: Each Scene independent layout + cue uses fade-up + scene switch = full-page opacity change = **voiceover PowerPoint** = quality zero. Full rules in `references/voiceover-pipeline.md` "Iron Rules" section.
...
| Animation-PowerPoint | Each scene independent layout + cue uses fade-up + full-page opacity switch (= voiceover PowerPoint) | **Whole piece is continuous motion narrative**: pick 1-2 hero elements that persist across scenes, each scene is hero's state change, scenes morph not cut (see `references/voiceover-pipeline.md` "Iron Rules") |
...
| **Long narration animation / concept video** (5-20 min voiceover, TTS-generated timeline, narration drives visuals) | `references/voiceover-pipeline.md` (iron rules: continuous motion narrative, no PowerPoint cuts) + `assets/narration_stage.jsx` + `scripts/{tts-doubao,narrate-pipeline}.mjs` + `scripts/{mix-voiceover,render-narration}.sh` |
...
- **Making voiceover animation** (≥1 min, long concept video): **Entire piece is one continuous motion narrative, not a set of independent scenes**. Pick 1-2 hero elements that persist across scenes, scenes morph not cut. Each Scene independent layout + cue using fade-up + full-page opacity switch = voiceover PowerPoint = quality zero. Full rules in `references/voiceover-pipeline.md` "Iron Rules" section. **This rule cannot be emphasized enough**.